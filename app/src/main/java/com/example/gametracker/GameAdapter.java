...
public class GameAdapter extends FirestoreRecyclerAdapter<Game, GameHolder> {
     ...
     * @param options
     */
    public GameAdapter(@NonNull FirestoreRecyclerOptions<Game> options, Context context) {
        super(options);
            ...
            if (user.isEmailVerified()){
                ...
                docRef.get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                    ...
                    public void onSuccess(DocumentSnapshot documentSnapshot) {
                        ...
                        assert user1 != null;
                        if (!(user1.getMyList().contains(title))) {
                            user1.setMyList(user1.getMyList() + "," + title);
                            db.collection("users").document(Objects.requireNonNull(user.getEmail())).set(user1);
                            Toast.makeText(context.getApplicationContext(), "Gra została dodana do twojej listy",
                                    Toast.LENGTH_SHORT).show();
                        }else{
                            Toast.makeText(context.getApplicationContext(), "Gra już była na twojej liście",
                                    Toast.LENGTH_SHORT).show();
                        }
                    }
                });
